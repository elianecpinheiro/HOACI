...
main()
{
  ...
  vEMV[ci-fail][] = muhat - up*stderrors[0];
  ...
  vMBR[ci-fail][] = muhatMBR - up*stderrorsMBR[0];
  ...
  for(i = 0; i < lprob ; i++)
  {
	...
	vQBR[ci-fail][i] = muhatQBR;
  }

  /* Coverage  */
  for(i = 0; i < idiv(lprob,2)+1 ; i++)	/* prob < 0.5 */
  {
	coverageEMV_L[ci-fail][i] = (vEMV[ci-fail][lprob-i-1] .<= mu);
	coverageMBR_L[ci-fail][i] = (vMBR[ci-fail][lprob-i-1] .<= mu);
	coverageQBR_L[ci-fail][i] = (vQBR[ci-fail][lprob-i-1] .<= mu);

	coverageEMV_U[ci-fail][i] = (mu .<= vEMV[ci-fail][i]);
	coverageMBR_U[ci-fail][i] = (mu .<= vMBR[ci-fail][i]);
	coverageQBR_U[ci-fail][i] = (mu .<= vQBR[ci-fail][i]);

	coverageEMV_B[ci-fail][i] = (vEMV[ci-fail][lprob-i-1] .<= mu)&&(mu .<= vEMV[ci-fail][i]);
	coverageMBR_B[ci-fail][i] = (vMBR[ci-fail][lprob-i-1] .<= mu)&&(mu .<= vMBR[ci-fail][i]);
	coverageQBR_B[ci-fail][i] = (vQBR[ci-fail][lprob-i-1] .<= mu)&&(mu .<= vQBR[ci-fail][i]);
